...
def filter_by_year(statistics, year, yearid):
    ...
    """
    return list(filter(lambda stat: int(stat[yearid]) == year, statistics))
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    return sorted(
                list(
                    map(
                        lambda stat: (stat[info['playerid']],formula(info, stat)),
                        statistics
                    )
                ),
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    str_list = []
    players = read_csv_as_nested_dict(info['masterfile'],
                info['playerid'], info['separator'], info['quote'])
    for stat in top_ids_and_stats:
        fname = players[stat[0]][info['firstname']]
        lname =  players[stat[0]][info['lastname']]
        stat_val = stat[1]
        str_list.append(f'{stat_val:.3f} --- {fname} {lname}')
        #str_list.append(str() + ' --- ' + fname + ' ' + lname)
    return str_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    full_stats = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    year_stats = filter_by_year(full_stats, year, info['yearid'])
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for stat in statistics:
        stat_id = stat[playerid]
        if stat_id not in totals:
            totals[stat_id] = {playerid:stat_id}
        for field in fields:
            if field not in totals[stat_id]:
                totals[stat_id][field] = int(stat[field])
            else:
                totals[stat_id][field] += int(stat[field])
    return totals
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    players_stats = aggregate_by_player_id(statistics, info['playerid'], info['battingfields'])
    top_players = top_player_ids(info, list(players_stats.values()), formula, numplayers)
    return lookup_player_names(info, top_players)
...
#statistics = read_csv_as_list_dict(baseballdatainfo[battingfile],
#baseballdatainfo[separator], baseballdatainfo[quote])
#print(statistics)
